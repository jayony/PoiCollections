...
import com.android.compaty.util.function.Predicate;
...
> extends POITextExtractor {
    ...
    private void printComments(final Slide<S,P> slide, final Consumer<String> consumer) {
        List<? extends Comment> comments = slide.getComments();
        for (Comment comment : comments) {
            consumer.accept(comment.getAuthor() + " - " + comment.getText());
        }
    }
...
//            );
    ...
    private static boolean filterFonts(Object o, String typeface, Boolean italic, Boolean bold) {
        ...
        return
            typeface.equalsIgnoreCase(tr.getPoiFont().getFamily()) &&
