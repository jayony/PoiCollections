...
public class DrawBackground extends DrawShape {
    ...
    public void draw(Graphics2D graphics) {
        ...
        PoiPaint fill = dp.getPaint(graphics, getShape().getFillStyle().getPaint());
        Rectangle2D anchor2 = getAnchor(graphics, anchor);
        
        if(fill != null) {
            graphics.setRenderingHint(Drawable.GRADIENT_SHAPE, anchor);
            graphics.setPoiPaint(fill);
            if (fill instanceof DrawTexturePaint) {
                graphics.drawImage(((DrawTexturePaint) fill).getImage(), (int)anchor2.getX(), (int)anchor2.getY(),
                        (int)anchor2.getWidth(), (int)anchor2.getHeight(), null);
