...
public class HemfGraphics extends HwmfGraphics {
    ...
    public void updateWindowMapMode() {
        ...
        for (TransOperand operand : transOper) {
            operand.fun.accept(tx, iter.next());
        }
    ...
    protected PoiPaint getLinearGradient() {
        ...
        float[] keys = new float[colorsH.size()];
        Color[] values = new Color[colorsH.size()];
        for (int i = 0; i < colorsH.size(); i++) {
            Map.Entry<Float, Color> entry = colorsH.get(i);
            keys[i] = entry.getKey().floatValue();
