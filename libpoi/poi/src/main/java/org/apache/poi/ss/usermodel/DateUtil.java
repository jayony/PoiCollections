...
//            .toFormatter();
    ...
    private static final List<SimpleDateFormat> FORMATS = new ArrayList<SimpleDateFormat>() {
        {
            add(new SimpleDateFormat("[dd MMM[ yyyy]][[ ]h:m[:s] a][[ ]H:m[:s]]"));
            add(new SimpleDateFormat("[[yyyy ]dd-MMM[-yyyy]][[ ]h:m[:s] a][[ ]H:m[:s]]"));
            add(new SimpleDateFormat("[M/dd[/yyyy]][[ ]h:m[:s] a][[ ]H:m[:s]]"));
            add(new SimpleDateFormat("[[yyyy/]M/dd][[ ]h:m[:s] a][[ ]H:m[:s]]"));
        }
    };
     ...
     */
    public static double getExcelDate(Calendar calendar, boolean use1904windowing) {
    ...
    private static ThreadLocal<Integer> lastFormatIndex = lastFormatIndex();
