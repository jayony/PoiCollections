...
public final class ExtSSTRecord extends ContinuableRecord {
    ...
    public ExtSSTRecord(ExtSSTRecord other) {
        ...
        _sstInfos = null;
        if (other._sstInfos != null) {
            _sstInfos = new InfoSubRecord[other._sstInfos.length];
            for (int i = 0; i < other._sstInfos.length; i++) {
                _sstInfos[i] = new InfoSubRecord(other._sstInfos[i]);
            }
        }
    }
