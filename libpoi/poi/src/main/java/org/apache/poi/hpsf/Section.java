...
public class Section {
    ...
    public Section(final byte[] src, final int offset) throws UnsupportedEncodingException {
        ...
        if (cpOffset != null) {
            ...
            leis.setReadIndex(MathCompaty.toIntExact(this._offset + cpOffset));
        ...
        for (Map.Entry<Long,Long> me : offset2Id.entrySet()) {
            ...
            leis.setReadIndex(MathCompaty.toIntExact(this._offset + off));
        ...
        sectionBytes.write(src, MathCompaty.toIntExact(_offset), size);
        padSectionBytes();
    }
    ...
    private static int propLen(
        ...
        return MathCompaty.toIntExact(end - begin);
    }
    ...
    private boolean readDictionary(LittleEndianByteArrayInputStream leis, final int length, final int codepage) {
        ...
        for (int i = 0; i < nrEntries; i++) {
            ...
            int nrBytes = MathCompaty.toIntExact(((sLength-1) * (cp == CodePageUtil.CP_UNICODE ? 2 : 1)));
            if (nrBytes > 0xFFFFFF) {
