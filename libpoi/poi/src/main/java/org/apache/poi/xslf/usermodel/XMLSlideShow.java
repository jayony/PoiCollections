    ...
    protected void onDocumentRead() throws IOException {
        try {
            ...
            if (_presentation.isSetSldMasterIdLst()) {
                for (CTSlideMasterIdListEntry entry : _presentation.getSldMasterIdLst().getSldMasterIdList()) {
                    _masters.add(masterMap.get(entry.getId2()));
                }
...
//                );
            ...
            if (_presentation.isSetSldIdLst()) {
                for (CTSlideIdListEntry entry : _presentation.getSldIdLst().getSldIdList()) {
                    XSLFSlide sh = shIdMap.get(entry.getId2());
                    if (sh == null) {
                        LOG.log(POILogger.WARN, "Slide with r:id " + entry.getId() + " was defined, but didn't exist in package, skipping");
                    } else {
                        _slides.add(sh);
                    }
                }
...
//                });
    ...
    public List<XSLFPictureData> getPictureData() {
        if (_pictures.isEmpty()) {
            List<PackagePart> parts = getPackage().getPartsByName(Pattern.compile("/ppt/media/.*?"));
...
//            .mapToLong(CTSlideIdListEntry::getId).max();
        ...
        long max = 0;
        for (CTSlideIdListEntry entry : slideList.getSldIdArray()) {
            max = Math.max(max, entry.getId());
        }
        final XSLFRelation relationType = XSLFRelation.SLIDE;
        final int slideNumber = (int)(Math.max(max,255)+1);
