...
public class HemfPicture implements Iterable<HemfRecord>, GenericRecord {
    ...
    public List<HemfRecord> getRecords() {
        if (!isParsed) {
            ...
            HemfRecordIterator iterator = new HemfRecordIterator(stream);
            while (iterator.hasNext()) {
                HemfRecord next = iterator.next();
                if (next instanceof HemfHeader) {
                    header[0] = (HemfHeader) next;
                }
                next.setHeader(header[0]);
                records.add(next);
            }
        }
        return records;
