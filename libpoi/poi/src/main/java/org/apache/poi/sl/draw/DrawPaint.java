...
import com.android.compaty.util.function.BiFunction;
...
public class DrawPaint {
    ...
    public PoiPaint getPaint(Graphics2D graphics, PaintStyle paint) {
        return getPaint(graphics, paint, PaintModifier.NORM);
    }

    public PoiPaint getPaint(Graphics2D graphics, PaintStyle paint, PaintModifier modifier) {
    ...
    protected PoiPaint getSolidPaint(SolidPaint fill, Graphics2D graphics, final PaintModifier modifier) {
    ...
    protected PoiPaint getGradientPaint(GradientPaint fill, Graphics2D graphics) {
    ...
    @SuppressWarnings("WeakerAccess")
    protected PoiPaint getTexturePaint(TexturePaint fill, Graphics2D graphics) {
        ...
        try (InputStream is = fill.getImageData()) {
            ...
            if (flip != null && flip != FlipMode.NONE) {
                ...
                final BufferedImage img = BufferedImage.create((int)(width*flipX), (int)(height*flipY));
                Graphics2D g = img.createGraphics();
                g.drawImage(image, 0, 0, (int)(width*flipX), (int)(height*flipY), paint);

                switch (flip) {
                    case X:
                        g.drawImage(image, 2*width, 0, -width, height, paint);
                        break;
                    case Y:
                        g.drawImage(image, 0, 2*height, width, -height, paint);
                        break;
                    case XY:
                        g.drawImage(image, 2*width, 0, -width, height, paint);
                        g.drawImage(image, 0, 2*height, width, -height, paint);
                        g.drawImage(image, 2*width, 2*height, -width, -height, paint);
                        break;
                }
...
//        return patIdx;
    ...
    @SuppressWarnings("WeakerAccess")
    protected PoiPaint createLinearGradientPaint(GradientPaint fill, Graphics2D graphics) {
...
//        snapToAnchor(p2, anchor);
    ...
    @SuppressWarnings("WeakerAccess")
    protected PoiPaint createRadialGradientPaint(GradientPaint fill, Graphics2D graphics) {
        Rectangle2D anchor = DrawShape.getAnchor(graphics, shape);
        if (anchor == null) {
            return TRANSPARENT;
        }
    ...
    @SuppressWarnings({"WeakerAccess", "unused"})
    protected PoiPaint createPathGradientPaint(GradientPaint fill, Graphics2D graphics) {
        // currently we ignore an eventually center setting

        return safeFractions(PathGradientPaint::new, fill);
    }

    private PoiPaint safeFractions(BiFunction<float[],Color[], PoiPaint> init, GradientPaint fill) {
        // if style is null, use transparent color to get color of background
        ColorStyle[] colors = fill.getGradientColors();
        List<Color> colorList = new ArrayList<>();
        for (ColorStyle color : colors) {
            if (color == null) {
                colorList.add(TRANSPARENT);
            } else {
                colorList.add(applyColorTransform(color));
            }
        }
        final Iterator<Color> styles = colorList.iterator();/*Stream.of(fill.getGradientColors())
    ...
    private static float[] toArray(Collection<Float> floatList) {
        ...
        Float[] floats = floatList.toArray(new Float[0]);
        for (int i = 0; i < floatList.size(); i++) {
            ret[i] = floats[i].floatValue();
        }
        return ret;
    }
