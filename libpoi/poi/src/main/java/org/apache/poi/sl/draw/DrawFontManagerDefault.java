...
public class DrawFontManagerDefault implements DrawFontManager {
    ...
    public FontInfo getFallbackFont(Graphics2D graphics, FontInfo fontInfo) {
        ...
        if (fi == null) {
            fi = new DrawFontInfo(new PoiFont());
        }
        return fi;
    }
    ...
    public String mapFontCharset(Graphics2D graphics, FontInfo fontInfo, String text) {
        ...
        return (fontInfo != null && knownSymbolFonts.contains(fontInfo.getFamily()))
            ? mapSymbolChars(text)
            : text;
    }
    ...
    @Override
    public PoiFont createAWTFont(Graphics2D graphics, FontInfo fontInfo, int fontSize, boolean bold, boolean italic) {
        int style = (bold ? Typeface.BOLD : 0) | (italic ? Typeface.ITALIC : 0);
        PoiFont font = new PoiFont(fontInfo.getFamily(), style, fontSize);
        return font;
    }
    ...
    private FontInfo getFontWithFallback(Graphics2D graphics, DrawableHint hint, FontInfo fontInfo) {
        ...
        String f = (fontInfo != null) ? fontInfo.getFamily() : null;
        ...
        return (mappedTypeface != null) ? new DrawFontInfo(PoiFont.createFont(mappedTypeface)) : fontInfo;
    }
}
