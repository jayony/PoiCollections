...
public class HwmfImageRenderer implements ImageRenderer, EmbeddedExtractor {
    ...
    public BufferedImage getImage(Dimension2D dim) {
        ...
        BufferedImage bufImg = BufferedImage.create((int)dim.getWidth(), (int)dim.getHeight());
    ...
    public static Iterable<EmbeddedPart> getEmbeddings(Iterable<HwmfEmbedded> embs) {
        return () -> {
            ...
            final int[] idx = { 1 };
            return new Iterator<EmbeddedPart>() {
                @Override
                ...
                @Override
                public EmbeddedPart next() {
                    EmbeddedPart ep = new EmbeddedPart();
