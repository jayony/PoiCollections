...
public class GenericRecordXmlWriter implements Closeable {
    ...
    protected boolean writeProperties(GenericRecord record) {
        ...
        childIndex = 0;
        List<Map.Entry<String,Supplier<?>>> complex = new ArrayList<>();
        for (Map.Entry<String, Supplier<?>> entry : prop.entrySet()) {
            Map.Entry<String, Supplier<?>> writeProp = writeProp(entry);
            if (writeProp != null) {
                complex.add(writeProp);
            }
        }
        attributePhase = false;
        if (!complex.isEmpty()) {
            fw.println(">");
            indent++;
            for (Map.Entry<String, Supplier<?>> entry : complex) {
                writeProp(entry);
            }
            indent--;
        }
    ...
    protected Map.Entry<String,Supplier<?>> writeProp(Map.Entry<String,Supplier<?>> me) {
        ...
        if (attributePhase == isComplex) {
            return isComplex ? new AbstractMap.SimpleEntry<>(me.getKey(), () -> obj) : null;
        }
    ...
    protected void writeValue(String name, Object value) {
        ...
        if (value instanceof GenericRecord) {
            ...
            for (Map.Entry<Class, GenericRecordHandler> entry : handler) {
                if (matchInstanceOrArray(entry.getKey(), value)) {
                    entry.getValue().print(this, name, value);
                    break;
                }
            }
        }
...
//        ((List)o).forEach(e -> { writeValue("item>", e); childIndex++; });
    ...
    protected boolean printBytes(String name, Object o) {
        assert (!attributePhase);
        openName(name+">");
        fw.write(new Base64().encodeAsString((byte[]) o));
        closeName(name+">");
        return true;
    }
