...
public final class HSLFFill {
    ...
    private PaintStyle getPaintStyle() {
        ...
        if (!FILL_USE_FILLED.isSet(propVal) && masterProp != null) {
            ...
            for (HSLFShape hslfShape : shape.getSheet().getMasterSheet().getShapes()) {
                if (hslfShape.getShapeId() == masterId) {
                    o = hslfShape;
                    break;
                }
            }
    ...
    private GradientPaint getGradientPaint(final GradientType gradientType) {
        ...
        if (colorCnt == 0) {
            ...
            for (byte[] bytes : ep) {
                EscherColorRef ecr = new EscherColorRef(bytes, 0, 4);
                colors.add(shape.getColor(ecr));
                double pos = Units.fixedPointToDouble(LittleEndian.getInt(bytes, 4));
        ...
        return new GradientPaint() {
            ...
            public ColorStyle[] getGradientColors() {
                List<ColorStyle> styleList = new ArrayList<>();
                for (Color color : colors) {
                    styleList.add(wrapColor(color));
                }
                return styleList.toArray(new ColorStyle[0]);
            }
