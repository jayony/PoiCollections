...
import com.android.compaty.util.function.Consumer;
...
public class DrawSimpleShape extends DrawShape {
    ...
    public void draw(Graphics2D graphics) {
        ...
        PoiPaint oldPoiPaint = graphics.getPoiPaint();
        Stroke oldStroke = graphics.getStroke();
        Color oldColor = graphics.getColor();

        PoiPaint fill = getFillPaint(graphics);
        PoiPaint line = getLinePaint(graphics);
        ...
        graphics.setPoiPaint(oldPoiPaint);
    ...
    private void fillArea(Graphics2D graphics, PaintModifier pm, Path2D area) {
        ...
        final PoiPaint fillMod = drawPaint.getPaint(graphics, ps, pm);
        if (fillMod != null) {
            graphics.setPoiPaint(fillMod);
            fillPaintWorkaround(graphics, area);
        }
    }

    protected PoiPaint getFillPaint(Graphics2D graphics) {
    ...
    protected PoiPaint getLinePaint(Graphics2D graphics) {
        final PaintStyle ps = getShape().getFillStyle().getPaint();
        DrawPaint drawPaint = DrawFactory.getInstance(graphics).getPaint(getShape());
    ...
    protected void drawDecoration(Graphics2D graphics, PoiPaint line, BasicStroke stroke) {
        if(line == null) {
            return;
        }
        graphics.setPoiPaint(line);
    ...
    protected void drawShadow(
      ...
      , PoiPaint line
        ...
        for (Outline o : outlines) {
            ...
            Path p = o.getPath();
            graphics.setRenderingHint(Drawable.GRADIENT_SHAPE, s);
            graphics.setPoiPaint(shadowColor);
