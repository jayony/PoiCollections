...
public abstract class AbstractEscherOptRecord extends EscherRecord {
    ...
    public <T extends EscherProperty> T lookup( int propId ) {
        for (EscherProperty property : properties) {
            if (property.getPropertyNumber() == propId) {
                return (T) property;
            }
        }
        return null;
    ...
    public void sortProperties() {
        Collections.sort(properties, new Comparator<EscherProperty>() {
            @Override
            public int compare(EscherProperty o1, EscherProperty o2) {
                return o1.getPropertyNumber() - o2.getPropertyNumber();
            }
        });
    }
    ...
    public void setEscherProperty(EscherProperty value){
        final Iterator<EscherProperty> iterator = properties.iterator();
        while (iterator.hasNext()) {
            EscherProperty next = iterator.next();
            if (next.getId() == value.getId()) {
                iterator.remove();
            }
        }
    ...
    public void removeEscherProperty(EscherPropertyTypes type){
        removeEscherProperty(type.propNumber);
    }
