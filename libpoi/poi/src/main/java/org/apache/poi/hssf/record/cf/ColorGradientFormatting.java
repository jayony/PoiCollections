...
public final class ColorGradientFormatting implements Duplicatable {
    ...
    public ColorGradientFormatting(ColorGradientFormatting other) {
        ...
        if (other.thresholds != null) {
            thresholds = new ColorGradientThreshold[other.thresholds.length];
            for (int i = 0; i < other.thresholds.length; i++) {
                thresholds[i] = other.thresholds[i].copy();
            }
        }
        if (other.colors != null) {
            colors = new ExtendedColor[other.colors.length];
            for (int i = 0; i < other.colors.length; i++) {
                colors[i] = other.colors[i].copy();
            }
        }
    }
