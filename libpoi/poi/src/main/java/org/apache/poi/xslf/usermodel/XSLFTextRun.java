...
public class XSLFTextRun implements TextRun {
    ...
    private Double getFontSizeFromProperty() {
        double scale = 1;
    ...
    private void updateFontSize() {
        Double property = getFontSizeFromProperty();
        poiFont.setSize(property == null ? -1 : property.floatValue());
    }
    ...
    void copy(XSLFTextRun r){
        Typeface typeface = r.getPoiFont().getTypeface();
        if(typeface != null && typeface.equals(getPoiFont().getTypeface())){
            setPoiFont(r.getPoiFont());
        }
