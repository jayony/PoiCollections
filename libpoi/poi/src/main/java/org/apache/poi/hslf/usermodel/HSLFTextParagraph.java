...
public final class HSLFTextParagraph implements TextParagraph<HSLFShape,HSLFTextParagraph,HSLFTextRun> {
    ...
    * @param parentList the list which contains this paragraph
    ...
    @Override
    public PoiFont getDefaultPoiFont() {
        PoiFont poiFont = null;
        if (!_runs.isEmpty()) {
            HSLFTextRun tr = _runs.get(0);
            poiFont = tr.getPoiFont();
    ...
    public Double getDefaultFontSize() {
        return Double.valueOf(getDefaultPoiFont().getSize());
    }
...
//        fi = getSheet().getSlideShow().addFont(fi);
        ...
        setParagraphTextPropVal("bullet.font", 0/*fi.getIndex()*/);
...
//        return ppFont.getTypeface();
    ...
    public List<? extends TabStop> getTabStops() {
        ...
        List<HSLFTabStopDecorator> list = new ArrayList<>();
        for (HSLFTabStop stop : tabStops) {
            list.add(new HSLFTabStopDecorator(stop));
        }
        return list;
    }
