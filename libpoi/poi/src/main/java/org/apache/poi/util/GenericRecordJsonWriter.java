...
public class GenericRecordJsonWriter implements Closeable {
    ...
    protected boolean writeProperties(GenericRecord record) {
        ...
        for (Map.Entry<String, Supplier<?>> entry : prop.entrySet()) {
            if (writeProp(entry.getKey(), entry.getValue())) {
                cnt++;
            }
        }
    ...
    protected boolean writeChildren(GenericRecord record, boolean hasProperties) {
        ...
        for (GenericRecord genericRecord : list) {
            if (writeValue(null, genericRecord) && ++childIndex > 0) {
                cnt++;
            }
        }
        childIndex = oldChildIndex;
    ...
    protected boolean writeValue(String name, Object o) {
        ...
        if (o == null) {
            grh = GenericRecordJsonWriter::printNull;
        } else {
            for (Map.Entry<Class, GenericRecordHandler> entry : handler) {
                if (matchInstanceOrArray(entry.getKey(), o)) {
                    grh = entry.getValue();
                    break;
                }
            }
    ...
    protected boolean printBytes(String name, Object o) {
        printName(name);
        fw.write('"');
        fw.write(Base64.encodeToString((byte[]) o, Base64.NO_PADDING));
        fw.write('"');
        return true;
    }
