...
import com.android.compaty.util.function.Function;
...
public enum LocaleID {
    ...
    private static final Map<String, LocaleID> languageTagLookup = languageTagLookup();

    private static Map<String, LocaleID> languageTagLookup() {
        Map<String, LocaleID> map = new LinkedHashMap<>();
        for (LocaleID value : values()) {
            if (value.isValid()) {
                map.put(value.getLanguageTag(), value);
            }
        }
        return map;
    }

    private static final Map<Integer, LocaleID> lcidLookup = lcidLookup();


    private static Map<Integer, LocaleID> lcidLookup() {
        Map<Integer, LocaleID> map = new LinkedHashMap<>();
        for (LocaleID value : values()) {
            map.put(value.getLcid(), value);
        }
        return map;
    }
