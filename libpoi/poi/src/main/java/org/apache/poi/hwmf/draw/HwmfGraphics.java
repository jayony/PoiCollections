...
import com.android.compaty.util.function.BiConsumer;
...
public class HwmfGraphics {
    ...
    public void fill(Shape shape) {
        ...
        if (prop.getBrushStyle() != HwmfBrushStyle.BS_NULL) {
            if (prop.getBkMode() == HwmfBkMode.OPAQUE) {
                graphicsCtx.setPoiPaint(prop.getBackgroundColor().getColor());
                graphicsCtx.fill(shape);
            }

            graphicsCtx.setPoiPaint(getFill());
            graphicsCtx.fill(shape);
        }
    ...
    protected PoiPaint getLinearGradient() {
        return null;
    }

    protected PoiPaint getSolidFill() {
        return getProperties().getBrushColor().getColor();
    }

    protected PoiPaint getHatchedFill() {
        int dim = 7, mid = 3;
        BufferedImage bi = BufferedImage.create(dim, dim);
    ...
    protected PoiPaint getPatternPaint() {
        HwmfDrawProperties prop = getProperties();
...
//        addAttributes(as, font, fontInfo.getFamily());
        SpannableString spannable = new SpannableString(textString);
        addSpan(spannable, font, fontInfo.getFamily());
...
//        final TextLayout layout = new TextLayout(as.getIterator(), frc);
        TextPaint paint = getPaint(font);
        float leading = paint.getFontMetrics().leading;
        double width = Math.ceil(StaticLayout.getDesiredWidth(spannable, paint)) + 1;
        StaticLayout layout = new StaticLayout(spannable, paint, (int)width, Layout.Alignment.ALIGN_NORMAL,
                1f, 0f, false);
        final Rectangle2D pixelBounds = new Rectangle(0, 0, (int)width, layout.getHeight());
        ...
        switch (valign) {
            case TOP:
                tx.translate(0, leading);
            default:
            case BASELINE:
                break;
            case BOTTOM:
                tx.translate(0, -(pixelBounds.getHeight()-leading));
                break;
        }
        tx.rotate(angle);
        ...
        try {
            ...
            graphicsCtx.drawLayout(layout, 0, 0);
        } finally {
            graphicsCtx.setTransform(at);
    ...
    private void addSpan(SpannableString spannable, HwmfFont font, String family) {
        ...
        if (awtFW > TextAttribute.WEIGHT_REGULAR) {
            style |= Typeface.BOLD;
        }
        int length = spannable.length();
         int fontSize = (int)(getFontHeight(font) * (1 + (awtFW / 10)));
        spannable.setSpan(new TextAppearanceSpan(family, style, fontSize,
                null, null), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        if (font.isUnderline()) {
            spannable.setSpan(new UnderlineSpan(), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        if (font.isStrikeOut()) {
            spannable.setSpan(new StrikethroughSpan(), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }

    private TextPaint getPaint(HwmfFont font) {
        TextPaint textPaint = new TextPaint();
        textPaint.setStyle(Paint.Style.FILL);
        textPaint.setAntiAlias(true);
        textPaint.setDither(true);
        textPaint.setTextSize((int)getFontHeight(font));
        return textPaint;
    }
    ...
    public void drawImage(ImageRenderer img, Rectangle2D srcBounds, Rectangle2D dstBounds) {
        ...
        switch (prop.getRasterOp()) {
            ...
            case PATCOPY:
                ...
                break;
            case BLACKNESS:
                graphicsCtx.setPoiPaint(Color.BLACK);
                graphicsCtx.fill(dstBounds);
                break;
            case WHITENESS:
                graphicsCtx.setPoiPaint(Color.WHITE);
                graphicsCtx.fill(dstBounds);
                break;
            default:
            case SRCCOPY:
                ...
                if (prop.getBkMode() == HwmfBkMode.OPAQUE) {
                    PoiPaint oldPoiPaint = graphicsCtx.getPoiPaint();
                    graphicsCtx.setPoiPaint(prop.getBackgroundColor().getColor());
                    graphicsCtx.fill(dstBounds);
                    graphicsCtx.setPoiPaint(oldPoiPaint);
                }
